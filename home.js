(function(){
  ...
  /* ---------- mobile hero title: align all three lines to the same left/right edges ---------- */
  (function(){
    function alignMobileHeroTitle(){
      ...
      /* Start from the same base tracking, then add only the spacing needed
         to make every line exactly as wide as the longest line. */
      lines.forEach(function(line){
        line.style.letterSpacing = '';
      });
      ...
      var target = Math.max.apply(Math, widths);

      lines.forEach(function(line, i){
        var text = (line.textContent || '').replace(/\s+/g, '');
        var gaps = Math.max(1, text.length - 1);
        var computed = window.getComputedStyle(line);
        var current = parseFloat(computed.letterSpacing);
        if(!isFinite(current)) current = 0;

        var extra = (target - widths[i]) / gaps;
        line.style.letterSpacing = (current + extra) + 'px';
      });
    }

    function scheduleHeroTitleAlign(){
      window.requestAnimationFrame(function(){
        window.requestAnimationFrame(alignMobileHeroTitle);
      });
    }

    scheduleHeroTitleAlign();
    window.addEventListener('resize', scheduleHeroTitleAlign);
    window.addEventListener('orientationchange', scheduleHeroTitleAlign);
    if(document.fonts && document.fonts.ready){
      document.fonts.ready.then(scheduleHeroTitleAlign);
    }
  })();
